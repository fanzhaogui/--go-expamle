...
// @code 小程序登录时获取的 code
res, err := wechat.Login(appID, secret, code)
if err != nil {
    // handle error
}
...
phone , err := wechat.DecryptPhoneNumber(ssk, data, iv string)
...
//
// @gid 小程序唯一群号
openGid , err := wechat.DecryptShareInfo(ssk, data, iv string)
...
// @iv 加密算法的初始向量
// @ssk 微信 session_key
ui, err := wechat.DecryptUserInfo(rawData, encryptedData, signature, iv, ssk string)
if err != nil {
    return
}
