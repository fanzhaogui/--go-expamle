...
func List(offset uint, count uint, token string) (list []Template, total uint, err error) {
	return templates(wechat.BaseURL+listAPI, offset, count, token)
}
...
func Selves(offset uint, count uint, token string) (list []Template, total uint, err error) {
	return templates(wechat.BaseURL+selvesAPI, offset, count, token)
}
...
func Get(id, token string) (keywords []Keyword, err error) {
	api, err := util.TokenAPI(wechat.BaseURL+detailAPI, token)
	if err != nil {
		return
	}
...
func Add(id, token string, keywordIDList []uint) (string, error) {
	api, err := util.TokenAPI(wechat.BaseURL+addAPI, token)
	if err != nil {
		return "", err
	}
	...
	if res.StatusCode != 200 {
		return "", errors.New(wechat.WeChatServerError)
	}
...
func Delete(id, token string) error {
	api, err := util.TokenAPI(wechat.BaseURL+deleteAPI, token)
	if err != nil {
		return err
	}
